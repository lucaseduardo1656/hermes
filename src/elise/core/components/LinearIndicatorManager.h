...
// 1-1 port of Caelestia's plugin/src/Caelestia/Internal/linearindicatormanager
// — the M3 linear indeterminate progress state machine. Pure logic, no
// Quickshell deps. Registered into the Elise QML module.
...
class LinearIndicatorSegment : public QObject {
    Q_OBJECT
    QML_ELEMENT
    QML_UNCREATABLE("LinearIndicatorSegments can only be retrieved from a LinearIndicatorManager.")
...
class LinearIndicatorManager : public QObject {
    ...
    Q_PROPERTY(QList<LinearIndicatorSegment*> activeIndicators READ activeIndicators CONSTANT FINAL)

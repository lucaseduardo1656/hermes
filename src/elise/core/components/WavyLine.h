...
// 1-1 port of Caelestia's plugin/src/Caelestia/Components/wavyline.hpp — a
// QQuickPaintedItem that draws an animated sine wave along a line or arc. Used
// by StyledSlider / StyledProgressBar / CircularProgress. Self-contained
// (QPainter only); registered into the Elise QML module via QML_ELEMENT.
class WavyLine : public QQuickPaintedItem {
    ...
    Q_PROPERTY(qreal amplitudeMultiplier READ amplitudeMultiplier WRITE setAmplitudeMultiplier NOTIFY amplitudeMultiplierChanged FINAL)
